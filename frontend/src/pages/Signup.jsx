  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Errors can be arrays or strings; normalize
        const msgParts = [];
        if (data.username) msgParts.push(`Username: ${Array.isArray(data.username) ? data.username.join(", ") : data.username}`);
        if (data.password) msgParts.push(`Password: ${Array.isArray(data.password) ? data.password.join(", ") : data.password}`);
        if (data.email) msgParts.push(`Email: ${Array.isArray(data.email) ? data.email.join(", ") : data.email}`);
        setMsg(msgParts.join(" ") || JSON.stringify(data));
      }
    } catch (err) {
      console.error(err);
      setMsg("Signup failed. Check if backend is running.");
    }
  };
